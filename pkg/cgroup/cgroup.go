...
func parseCgroupController(pid int, controller string) ([]string, error) {
	var pidStr string
	if pid == -1 {
		pidStr = "self"
	} else {
	...
	cgroupPath := filepath.Join("/proc", pidStr, "cgroup")
	cg, err := os.Open(cgroupPath)
	if err != nil {
		return nil, fmt.Errorf("error opening /proc/%s/cgroup: %v", pidStr, err)
	}
	defer cg.Close()
	...
	for s.Scan() {
		...
		if len(parts) < 3 {
			return nil, fmt.Errorf("error parsing /proc/%s/cgroup", pidStr)
		}
...
func GetOwnCgroupPath(controller string) (string, error) {
	parts, err := parseCgroupController(-1, controller)
	if err != nil {
		return "", err
	}
	return parts[2], nil
