...
func NewACIInfo(blobKey string, latest bool, t time.Time, size int64) *ACIInfo {
	return &ACIInfo{
		BlobKey:    blobKey,
		Latest:     latest,
		ImportTime: t,
...
func aciinfoRowScan(rows *sql.Rows, aciinfo *ACIInfo) error {
	...
	return rows.Scan(&aciinfo.BlobKey, &aciinfo.Name, &aciinfo.ImportTime, &aciinfo.LastUsed, &aciinfo.Latest, &aciinfo.Size)
}
...
func WriteACIInfo(tx *sql.Tx, aciinfo *ACIInfo) error {
	...
	_, err = tx.Exec("INSERT into aciinfo (blobkey, name, importtime, lastused, latest, size) VALUES ($1, $2, $3, $4, $5, $6)", aciinfo.BlobKey, aciinfo.Name, aciinfo.ImportTime, aciinfo.LastUsed, aciinfo.Latest, aciinfo.Size)
	if err != nil {
		return err
	}
