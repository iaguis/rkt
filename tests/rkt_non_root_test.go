...
func TestNonRootReadInfo(t *testing.T) {
	...
	// Go disables setuid/setgid because of its runtime model make
	// these syscall not effactive in most case because:
	// 1. Only the kernal thread(not process) will successfully setuid/setgid
	// after the syscall.
	// 2. Kernal thread is not binded to goroutine.
	//
