...
func TestRunOverrideExec(t *testing.T) {
	...
	for i, tt := range []struct {
	...
	}{
		{
			...
			rktCmd:       fmt.Sprintf("%s --insecure-skip-verify run %s -- --print-exec", ctx.cmd(), execImage),
			expectedLine: "inspect execed as: /inspect",
		},
		{
			// Now test overridingt the entrypoint (which is a symlink to /inspect so should behave identically)
			rktCmd:       fmt.Sprintf("%s --insecure-skip-verify run %s --exec /inspect-link -- --print-exec", ctx.cmd(), execImage),
