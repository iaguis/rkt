...
// GetHash uses docker2aci library to download the image and convert
// it to ACI, then stores it in the store and returns the hash.
...
func (f *dockerFetcher) fetchImageFrom(u *url.URL, latest bool) (string, error) {
	...
	// TODO(krnowak): that ascURL from aciURL looks bogus for
	// docker images, it is not used anywhere I suppose
