...
// resumableSession is an implementation of downloadSession interface,
// it allows sending custom headers for authentication, resuming
// interrupted downloads, handling cache data.
type resumableSession struct {
	// InsecureSkipTLSVerify tells whether TLS certificate
	// validation should be skipped.
	InsecureSkipTLSVerify bool
	// Headers are HTTP headers to be added to HTTP request. Used
	// for authentication.
	Headers http.Header
	// File possibly holds the downloaded data - it is used for
	// resuming interrupted downloads.
	File *os.File
	// ETagFilePath is a path to file holding an ETag of a
	// downloaded file. It used for resuming interrupted
	// downloads.
	ETagFilePath string
	// Label is for printing a type of downloaded data when
	// printing a pretty progress bar.
	Label string
...
func (s *resumableSession) getMaxAge(headerValue string) int {
	...
	maxAge := 0
	// TODO(krnowak): Shouldn't we split it by commas and the
	// strip leading/trailing whitespaces from the parts?
	...
	for i := 0; i < len(parts); i++ {
		...
		case "no-store", "no-cache":
			maxAge = 0
			// TODO(krnowak): Shouldn't we just break out
			// of the loop at this point?
		case "max-age":
			secs, err := strconv.Atoi(val)
			if err != nil || secs != 0 && val[0] == '0' {
				// TODO(krnowak): Probably should set
				// maxAge to zero on error
				break
			}
			if secs <= 0 {
				maxAge = 0
